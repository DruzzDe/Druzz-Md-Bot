...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/arslan.html')) {
    console.log("✅ druzz.html found in lib folder");
  } else {
    console.log("⚠️ druzz.html not found in lib folder");
  }
...
const DEFAULT_WELCOME = "╭──⌬ *𝚆𝙴𝙻𝙲𝙾𝙼𝙴* ⌬──╮\n│\n├─⌬ *𝚄𝚜𝚎𝚛:* @user\n├─⌬ *𝙶𝚛𝚘𝚞𝚙:* @group\n├─⌬ *𝙼𝚎𝚖𝚋𝚎𝚛𝚜:* @count\n│\n╰──────────────────────⌬\n\n> 𝙴𝚗𝚓𝚘𝚢 𝚢𝚘𝚞𝚛 𝚜𝚝𝚊𝚢! 🎉";

// Default goodbye message
const DEFAULT_GOODBYE = "╭──⌬ *𝙶𝙾𝙾𝙳𝙱𝚈𝙴* ⌬──╮\n│\n├─⌬ *𝚄𝚜𝚎𝚛:* @user\n├─⌬ *𝙶𝚛𝚘𝚞𝚙:* @group\n├─⌬ *𝙻𝚎𝚏𝚝 𝚝𝚑𝚎 𝚐𝚛𝚘𝚞𝚙*\n│\n╰──────────────────────⌬\n\n> 𝚆𝚎'𝚕𝚕 𝚖𝚒𝚜𝚜 𝚢𝚘𝚞! 👋";
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("DRUZZ-MD~")) {
    console.log("❌ Invalid DRUZZ-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ DRUZZ-MD session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'DRUZZ'}</p>
        </div>
      </body>
      </html>
    `);
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://files.catbox.moe/cdyd0s.png'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 DRUZZ-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ DRUZZ-MD Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────⌬*\n> *𝙳𝚁𝚄𝚉𝚉-𝙼𝙳 𝙲𝙾𝙽𝙽𝙴𝙲𝚃𝙴𝙳 𝚂𝚄𝙲𝙲𝙴𝚂𝚂𝙵𝚄𝙻𝙻𝚈*\n\n> *𝚃𝚢𝚙𝚎 ${prefix}𝚖𝚎𝚗𝚞 𝚝𝚘 𝚟𝚒𝚎𝚠 𝚌𝚘𝚖𝚖𝚊𝚗𝚍𝚜*  \n\n*╭𖢰 𝙳𝚁𝚄𝚉𝚉-𝙼𝙳 𖢰╮*\n*│✫📂 𝙱𝚘𝚝 𝙽𝚊𝚖𝚎: ${botConfig.BOT_NAME}*\n*│✫🛡️ 𝙾𝚠𝚗𝚎𝚛: ${config.OWNER_NAME}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 𝙼𝚘𝚍𝚎: ${config.MODE}*\n*│✫⏰ 𝚄𝚙𝚝𝚒𝚖𝚎: ${runtime(process.uptime())}*\n*╰──────────────⌬*\n\n> 𝙴𝚗𝚓𝚘𝚢 𝚄𝚜𝚒𝚗𝚐 𝙺𝙸𝚁𝙰 𝙼𝙳`;
      
      // Image URL for connection message
      const imageUrl = 'https://files.catbox.moe/cdyd0s.png';
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '18095120668@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://files.catbox.moe/cdyd0s.png');
    ...
    try {
      ...
      if (isCmd && isGroup) {
        ...
        // SHOW WELCOME SETTINGS
          ...
          const settingsMsg = `╭──⌬ *𝚆𝙴𝙻𝙲𝙾𝙼𝙴 𝚂𝙴𝚃𝚃𝙸𝙽𝙶𝚂* ⌬──╮
│
...
├─⌬ *𝚃𝚒𝚖𝚎:* ${timeStr}
├─⌬ *𝙳𝚊𝚝𝚎:* ${dateStr}
├─⌬ *𝙲𝚑𝚊𝚝 𝚃𝚢𝚙𝚎:* ${chatType}${groupNameText}
│
├─⌬ *𝙳𝚎𝚕𝚎𝚝𝚎𝚍 𝙱𝚢:* @${deletedBy.split('@')[0]}
├─⌬ *𝙾𝚛𝚒𝚐𝚒𝚗𝚊𝚕 𝚂𝚎𝚗𝚍𝚎𝚛:* @${originalSender.split('@')[0]}
│
├─⌬ *𝙼𝚎𝚜𝚜𝚊𝚐𝚎 𝚃𝚢𝚙𝚎:* ${messageType}
├─⌬ *𝙲𝚘𝚗𝚝𝚎𝚗𝚝:* 
├─⌬ \`${originalContent.substring(0, 500)}${originalContent.length > 500 ? '...' : ''}\`
│
╰──────────────────────❍
        
> _𝙼𝚎𝚜𝚜𝚊𝚐𝚎 𝚠𝚊𝚜 𝚍𝚎𝚕𝚎𝚝𝚎𝚍 𝚋𝚞𝚝 𝚋𝚘𝚝 𝚜𝚊𝚟𝚎𝚍 𝚒𝚝_ 🔰`;
